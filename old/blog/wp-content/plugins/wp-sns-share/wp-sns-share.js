...
function share(url, title, sns){
	...
	if (sns == "renren") {
		...
		var source = '网易微博';
		shareURL = 'http://t.163.com/article/user/checkLogin.do?source='+ source +'&info='+ title + ' ' + url + '&' + new Date().getTime();
	} else if (sns == "tsohu") {
		var width = 700;
		shareURL = 'http://t.sohu.com/third/post.jsp?content=utf-8&title='+ title + '&url='+ url;
	} else if (sns == "fanfou") {
		...
		width = 800;
		shareURL = 'http://www.google.com/buzz/post?url='+ url + '&title='+ title;
